...
<section class="panel">
    <header class="panel-heading">
        Thêm mới danh mục sản phẩm con
    </header>
    <div class="panel-body">
        ...
        <form class="form-horizontal bucket-form" method="POST" action="{{ route('admin.categories.store') }}">
            @csrf
            <div class="form-group">
                <label class="col-sm-3 control-label">Tên danh mục con</label>
                <div class="col-sm-6">
                    <input type="text" id="categoryName" name="category_name" class="form-control" value="{{ old('category_name') }}" required>
                    @error('category_name')
                ...
                <div class="col-sm-6">
                    <select name="parent_categorie_id" class="form-control">
                        ...
                        <option value="{{ $parentCategory->parent_categorie_id }}"
                            {{ old('parent_categorie_id') == $parentCategory->parent_categorie_id ? 'selected' : '' }}>
                            {{ $parentCategory->name }}
                        </option>
                        @endforeach
                    </select>
